...
class DoMZeroPlayer:
    ...
    def dom_0_irl(self, prior:np.array, observation:int):
        p_0 = 1/2
    ...
    def dom_0_utility(self,belief):
        ...
        u_1 = -1 * np.sum(np.matmul(belief, self.dom_zero_utility_subroutine(0, policies_matrix)))
        u_2 = -1 * np.sum(np.matmul(belief, self.dom_zero_utility_subroutine(1, policies_matrix)))
        u_3 = -1 * np.sum(np.matmul(belief, self.dom_zero_utility_subroutine(2, policies_matrix)))
