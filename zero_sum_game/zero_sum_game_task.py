...
if __name__ == "__main__":    
    ...
    agents_dictionary = {"-2": random_agent, "-1": dom_m1_agent, "0": dom_zero_agent, "1": dom_one_agent, "2": dom_two_agent}
    path_to_data_dir = f"data/{payout_matrix_name}"
    os.makedirs(path_to_data_dir, exist_ok=True)        
    agents_list = [["-2", "-1", "1"], ["0", "2"]]
    for dyad in itertools.product(*agents_list):
        row_player = agents_dictionary[dyad[0]]
        column_player = agents_dictionary[dyad[1]]        
        simulate_row_column_task(payout_game, path_to_data_dir, duration, initial_beliefs, dyad, row_player, column_player, seed, save_results)
