...
results_path = "~/Max_Planck/Hierarchical-modelling/data/first_task/2_rational_agents/short_duration/"
dom_levels = list(
               c("0","-1"),
               c("0","1"))
...
sender_reward_plot <- 
  cumulative_reward_results %>% 
  ggplot(aes(sender_threshold, total_sender_reward, fill = sender_threshold)) + 
  geom_col() + 
  facet_grid(receiver_threshold~sender_dom_level) + 
  ylab("Cumulative sender reward") + 
  xlab("Trial number") + 
  scale_y_continuous(
      expand = c(0,0),
      name = "Sum of sender reward",
      limits = c(0,8),
      breaks = seq(0,8,1)
    ) +
  scale_fill_manual(values = brewer.pal(5, "Blues")[c(2,3,5)],
                      name = "Sender Theta", labels = c("0.3","0.7","Random"))

receiver_reward_plot <-
  cumulative_reward_results %>% 
  ggplot(aes(sender_threshold, total_receiver_reward, fill = sender_threshold)) + 
  geom_col() + 
...
```{r}
game_results %>% 
  filter(sender_dom_level == -1, receiver_threshold == 0.2, sender_threshold==0.0)
beliefs %>% 
  filter(sender_dom_level == 1, receiver_threshold == 0.2, sender_threshold==0.7,
          seed == 760)
```

```{r}
game_results %>% 
  filter(sender_dom_level == 1, receiver_threshold == 0.0, sender_threshold==0.7)
```
