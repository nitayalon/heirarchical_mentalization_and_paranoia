...
directory_pattern <- "DoM%s_receiver_DoM%s_sender_softmax_temp_0.05"
...
sender_reward_plot <- 
  ...
  scale_fill_manual(values = brewer.pal(5, "Blues")[c(3,5,7,9)],
                      name = "Sender Theta", labels = c("0.1","0.85","Random")) + 
   facet_grid(.~sender_dom_level) 
...
```{r}
first_task_data$game_results %>% 
  filter(sender_threshold == 0.75, receiver_threshold == 0.15)
```
