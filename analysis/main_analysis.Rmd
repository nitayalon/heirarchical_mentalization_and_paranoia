...
sender_reward_plot <- 
  ...
  scale_fill_manual(values = brewer.pal(5, "Blues")[c(3,5,7,9)],
                      name = "Sender Theta", labels = c("0.15","0.75","Random"))
...
## In depth analysis of the total reward

```{r}
first_task_data$game_results %>% 
  filter(sender_threshold == 0.75, receiver_threshold == 0.25)
...
typical_offer_sequence %>% 
  ...
  scale_color_manual(name = 'Sender threshold', values=wes_palette(n=4, name="Darjeeling1")) + 
  scale_fill_manual(name = 'Sender threshold', values=wes_palette(n=4, name="Darjeeling1")) + 
  facet_wrap(.~receiver_threshold)
```
